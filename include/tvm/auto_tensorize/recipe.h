...
/*!
 * \file auto_tensorize/recipe.h
 * \brief The definition of the "recipe" in the auto_schedule.
 *
 * Recipe.
 */

#ifndef TVM_AUTO_TENSORIZE_RECIPE_H_
#define TVM_AUTO_TENSORIZE_RECIPE_H_
...
/*!
 * \brief A recipe stage, describes how tensorize is done.
 * Multiple stages may refer to the same recipe stage, which
 * indicates that theese stages belong to the same recipe.
 */
class RecipeStageNode : public Object {
 public:
  ...
  String shape_key;
  /*! \brief Each operation has one capsule key */
  Map<te::Operation, String> capsule_key;
  ...
  TVM_DECLARE_FINAL_OBJECT_INFO(RecipeStageNode, Object);
};


class RecipeStage : public ObjectRef {
 public:
  /*!
  ...
  TVM_DEFINE_OBJECT_REF_METHODS(RecipeStage, ObjectRef, RecipeStageNode);
  TVM_DEFINE_OBJECT_REF_COW_METHOD(RecipeStageNode);
};
...
#endif  // TVM_AUTO_TENSORIZE_RECIPE_H_