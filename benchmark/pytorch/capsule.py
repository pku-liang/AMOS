        ...
        for j in range(number):
            ...
            for k in range(0, capsule_num):
                C_torch = torch.nn.functional.conv2d(
                    A_torch, B_torch, bias=None, stride=stride, padding=padding
                )
...
example_text = """
    example:
        python capsule.py --batch 16 --enable_cudnn --number 5 --repeats 5 --begin 0 --num 1 --dtype FP16
        python capsule.py --batch 8 --number 10 --repeats 10 --begin 0 --num 1 --dtype TF32
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--begin", type=int, choices=list(range(len(capsule_config))), default=0)
    parser.add_argument(
        "--num",
        type=int,
        choices=list(range(1, len(capsule_config) + 1)),
        default=len(capsule_config),
    )
    parser.add_argument(
        "--dtype",
        type=str,
    ...
    for i, shape in enumerate(capsule_config[beg : beg + num]):
        ...
        N = batch
        capsule(
            H, W, N, C, outC, kernel_size, stride, padding, capsule_num, args.dtype
        )
