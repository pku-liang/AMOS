...
def mapping_tensorcore(
    ...
    A, B, Conv = batch_conv2d(N, C, H, W, K, R, S, stride, padding, in_dtype, out_dtype)
    target_dag = at.compute_dag_from_tensors([Conv])
    target = "cuda"

    log_dir = "batch-conv2d-%s-%s-layer-%s" % (in_dtype, out_dtype, layer)
    log_file = "batch-conv2d-%s-%s-layer-%s.log" % (in_dtype, out_dtype, layer)
...
example_text = """
 example:
    python mapping_batchconv2d_tensorcore.py --in_dtype float16 --out_dtype float16 --begin 0 --num 1 --trials 20
    python mapping_batchconv2d_tensorcore.py --in_dtype float16 --out_dtype float32 --begin 0 --num 1 --trials 20
    python mapping_batchconv2d_tensorcore.py --in_dtype float32 --out_dtype float32 --begin 0 --num 1 --trials 20
    python mapping_batchconv2d_tensorcore.py --in_dtype float16 --out_dtype float16 --begin 0 --num 1 --trials 400 --simple_mode 0
"""
