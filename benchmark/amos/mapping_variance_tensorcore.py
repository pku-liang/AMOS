...
def mapping_tensorcore(
    ...
    A, B, Var = variance(N, C, H, W, in_dtype, out_dtype)
    target_dag = at.compute_dag_from_tensors([Var])
    target = "cuda"

    log_dir = "variance-%s-%s-layer-%s" % (in_dtype, out_dtype, layer)
    log_file = "variance-%s-%s-layer-%s.log" % (in_dtype, out_dtype, layer)
...
example_text = """
 example:
    python mapping_variance_tensorcore.py --in_dtype float16 --out_dtype float16 --begin 0 --num 1 --trials 20
    python mapping_variance_tensorcore.py --in_dtype float16 --out_dtype float32 --begin 0 --num 1 --trials 20
    python mapping_variance_tensorcore.py --in_dtype float32 --out_dtype float32 --begin 0 --num 1 --trials 20
    python mapping_variance_tensorcore.py --in_dtype float16 --out_dtype float16 --begin 0 --num 1 --trials 400 --simple_mode 0
"""
