...
def main(batch, dtype, trials):
    ...
    model = tensor_graph.testing.models.CapsuleNetwork(
        batch_size, dtype=in_dtype, out_dtype=out_dtype
    )
    model.eval()
    ...
    tid = dispatch.add_graph_task(
        "capsulenet", multi_graph, measure_opt, scheduler_option="auto_tensorize", trials=trials
    )
    ########################################
...
example_text = """
 example:
    python mapping_capsulenet_tensorcore.py --dtype float16 --trials 20
    python mapping_capsulenet_tensorcore.py --dtype float32 --trials 20
    python mapping_capsulenet_tensorcore.py --dtype float64 --trials 200
"""
