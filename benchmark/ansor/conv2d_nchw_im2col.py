...
def conv2d_layer(N, H, W, CO, CI, KH, KW, stride, padding):
    ...
    pad = te.compute(
        [N, CI, H + 2 * padding[0], W + 2 * padding[1]],
        lambda n, c, h, w:
            tvm.tir.if_then_else(
                tvm.tir.all(h >= padding[0], h < H + padding[0], w >= padding[1], w < W + padding[1]),
                data[n, c, h - padding[0], w - padding[1]],
                tvm.tir.const(0.0, data.dtype)
            ),
        name="padding"
    )
    P = (H + 2 * padding[0] - KH) // stride[0] + 1
    Q = (W + 2 * padding[1] - KW) // stride[1] + 1
    left = te.compute(
        [N*P*Q, CI*KH*KW],
        lambda npq, crs:
            pad[npq//(P*Q), crs//(KH*KW), npq%(P*Q)//Q*stride[0]+crs%(KH*KW)//KW, npq%Q*stride[1]+crs%KW],
        name="left"
    )
    right = te.compute(
        [CI*KH*KW, CO],
        lambda crs, k:
            kernel[k, crs//(KH*KW), crs%(KH*KW)//KW, crs%KW],
        name="right"
    )
    crs = te.reduce_axis([0, CI*KH*KW], name="crs")
    gemm = te.compute(
        [N*P*Q, CO],
        lambda npq, k:
            te.sum(
                left[npq, crs].astype("float32") * right[crs, k].astype("float32"),
                axis=[crs]),
        name="gemm"
    )
    conv = te.compute(
