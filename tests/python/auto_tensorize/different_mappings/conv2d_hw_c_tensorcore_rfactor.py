...
def conv2d_fuse_nhw_crs(N, C, H, W, K, R, S, stride, padding, dilation):
    ...
    BB = tvm.te.compute(
        [N, R, S, C, P*Q],
        lambda n, r, s, c, pq:
            padded[n, c, pq // Q * stride + r * dilation, pq % Q * stride + s],
        name="BB"
    )
    ...
    MO = ceil(K, intrin_M)
    NO = ceil(P*Q, intrin_N)
    KO = ceil(C, intrin_K)

    need_padding = (MO * intrin_M > K or NO * intrin_N > P*Q or KO * intrin_K > C)
    ...
    BBB = tvm.te.compute(
        [N, R, S, KO, NO, intrin_K, intrin_N],
        lambda n, r, s, ko, no, ki, ni:
            tvm.tir.if_then_else(
                tvm.tir.all(ko * intrin_K + ki < C, no * intrin_N + ni < P*Q),
                BB[n, r, s, ko * intrin_K + ki, no * intrin_N + ni],
    ...
    CCC = tvm.te.compute(
        [N, MO, NO, intrin_M, intrin_N],
        lambda n, mo, no, mi, ni:
            tvm.te.sum(
                (AAA[rr, rs, mo, rc // intrin_K, mi, rc % intrin_K] * BBB[n, rr, rs, rc // intrin_K, no, rc % intrin_K, ni]).astype("float32"),
                axis=[rr, rs, rc]
            ),
        name="CCC"
    )
    ...
    if need_padding:
        CC = tvm.te.compute(
            [N, K, P*Q],
            lambda n, k, pq:
                CCC[n, k//intrin_M, pq//intrin_N, k%intrin_M, pq%intrin_N] + CCC[N - 1, MO - 1, NO - 1, intrin_M - 1, intrin_N - 1],
            name="CC"
        )
    else:
        CC = tvm.te.compute(
            [N, K, P*Q],
            lambda n, k, pq:
                CCC[n, k//intrin_M, pq//intrin_N, k%intrin_M, pq%intrin_N],
            name="CC"
        )
...
def schedule_conv_fuse_nhw_crs(Image, Filter, Output):
    ...
    AAX = AAA
    BBX = BBB
    AAA = sch.cache_read(AAX, "shared", [CCC])
    BBB = sch.cache_read(BBX, "shared", [CCC])
    ...
    sch[CCC].reorder(n, m0, n0, m1, n1, m2, n2, mni0, mni1, vec)
    ...
    rk, n, mo, no, mi, ni = sch[RF].op.axis
    ...
    rk, n, mo, no, mi, ni = sch[LL].op.axis
    rr, rs, rco, rci = sch[LL].op.reduce_axis
    sch[LL].reorder(n, mo, no, rr, rs, rco, mi, ni, rci)
