...
def conv2d_fuse_nhw_crs(N, C, H, W, K, R, S, stride, padding, dilation):
    ...
    rk = tvm.te.reduce_axis([0, mmK], name="rk")
    rki = tvm.te.reduce_axis([0, intrin_K], name="rki")
    CCC = tvm.te.compute(
        [mmM, mmN, intrin_M, intrin_N],
        lambda mo, no, mi, ni:
            tvm.te.sum(
                (AAA[mo, rk, mi, rki] * BBB[rk, no, rki, ni]).astype("float32"),
                axis=[rk, rki]
            ),
        name="CCC"
    )
...
def schedule_conv_fuse_nhw_crs(Image, Filter, Output):
    ...
    # schedule LL
    rk, rki = sch[CCC].op.reduce_axis
    rk = sch[CCC].fuse(rk, rki)
