...
def main_process(
  ...
  profile_level=0, test_only=False, change_layout=False, number=10, repeats=100):
  os.environ["TG_PRINT_LEVEL"] = "4"

  if test_only:
    execution_log_file="tmp.txt"
    autoschedule_log_file="tmp_autoschedule_log.txt"
    autoschedule_profile_file="tmp_autoschedule_log_profile.txt"
    build_log_file="tmp_build_log.txt"
    execution_log_file="tmp_execution_log.txt"
...
if __name__ == "__main__":  
  ...
  main_exit_code = main_process(
    target, dev_id, args.delete, args.exe_iter, args.tune_iter, args.minutes, args.reference, args.save,
    args.first_stage, args.second_stage_ratio, args.no_run, args.profile, args.test, args.layout, args.eval_number, args.eval_repeat)
  try:
    ret = main_exit_code.result()
