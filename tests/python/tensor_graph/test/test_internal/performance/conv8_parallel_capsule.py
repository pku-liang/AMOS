...
from tvm.tensor_graph.testing.models import conv8_parallel
...
def consumer_process(target, dev_id, number=10, repeat=10, reference="", profile_level=0):
  ...
  # False means the convolution is fusioned in frontend already!
  model = conv8_parallel.get_model(False)
  batch = 1
  img_shape = [batch, 256, 20, 20]
