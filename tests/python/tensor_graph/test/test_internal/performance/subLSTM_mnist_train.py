...
from tvm.tensor_graph.testing.models import subLSTM
...
def consumer_process(target, dev_id, number=10, repeat=10, reference="", profile_level=0):
  ...
  # get the model
  batch_size = 64
  model = subLSTM.get_model()
  img_shape = [batch_size, 28*28]
  hc_shape = [batch_size, 128]
  label_shape = [batch_size, 10]
  dtype = "float32"

  img_tensor = GraphTensor(img_shape, dtype, name="data", requires_grad=False)
  old_h = GraphTensor(hc_shape, dtype, name="old_h", requires_grad=False)
  old_c = GraphTensor(hc_shape, dtype, name="old_c", requires_grad=False)
  ...
  old_h_data = np.zeros(hc_shape).astype(dtype)
  old_c_data = np.zeros(hc_shape).astype(dtype)
