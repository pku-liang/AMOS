...
from tvm.tensor_graph.testing.models import MI_LSTM
...
def consumer_process(target, dev_id, number=10, repeat=10, reference="", profile_level=0):
  ...
  # get the model
  batch_size = 64
  model = MI_LSTM.get_model()
  img_shape = [batch_size, 28*28]
  h_shape = [batch_size, 1024]
  c_shape = [batch_size, 1024]
  label_shape = [batch_size, 10]
  dtype = "float32"

  img_tensor = GraphTensor(img_shape, dtype, name="data", requires_grad=False)
  old_h = GraphTensor(h_shape, dtype, name="old_h", requires_grad=False)
  old_c = GraphTensor(c_shape, dtype, name="old_c", requires_grad=False)
  label_tensor = GraphTensor(label_shape, dtype, name="label", requires_grad=False)

  loss = LSTMCELoss(label_tensor)

  optimizer = SGD(lr=0.002)


  # get forward graph and tir graph
  fwd_graph = make_fwd_graph(model, [img_tensor, old_h, old_c])
  ...
  old_h_data = np.zeros(h_shape).astype(dtype)
  old_c_data = np.zeros(c_shape).astype(dtype)
  ...
  inputs_bindings = {
    ...
    tir_graph.inputs[1]: tvm.nd.array(old_h_data, ctx),
    tir_graph.inputs[2]: tvm.nd.array(old_c_data, ctx),
...
def producer_process(
  ...
  while True:
    if count_iter == 100:
        ...
        tmp_save_to = ""
    beg = time.time()
    tg.run_task(sess, task_id, [inputs_bindings] * number, save_to=tmp_save_to, no_actual_run=no_run, profile_level=1)
    end = time.time()
    count_iter += 1
    if no_run:
      time.sleep(10 * 60)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--exe_iter", help="max execution iter", type=int, default=20)
