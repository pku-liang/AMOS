...
B_np = np.random.uniform(-1, 1, [K, IG, R, S]).astype("float32")
...
golden_torch = torch.nn.functional.conv_transpose2d(dC_torch, B_torch, stride=(st, st), output_padding=1, groups=group)
testing.assert_allclose(dA_tvm.asnumpy(), golden_torch.numpy(), rtol=1e-3, atol=1e-30)
