...
from tvm.tensor_graph.nn import CELoss, Adam#, SGD
import argparse
...
def consumer_process(target, dev_id, number=10, repeat=10, reference="", profile_level=0):
  ...
  tir_graph = make_tir_graph(fwd_graph, inference=False, loss=loss, optimizer=optimizer, need_output=True, need_grad=False)
  ...
  output_keys = [tir_graph.outputs[0], # prediction
                 tir_graph.loss # loss
                ]
  ...
  for epoch in range(num_epoches):
      ...
      for i, data in enumerate(train_loader):
          ...
          label_numpy = label_torch.numpy().astype(dtype)
          img = np.pad(img, ((0,0),(0,0), (2,2),(2,2)), 'constant')


          inputs_data = img
          label_data = label_numpy

          if first_iteration == True:
            first_iteration = False
            for k in range(len(optimizer.old_m_list)):
              shape = optimizer.old_m_list[k].shape
              # This binding is incorrect! Should be revised later
              optimizer.old_m_list[k] = tvm.nd.array(np.zeros(shape).astype("float32"), ctx)
              optimizer.old_v_list[k] = tvm.nd.array(np.zeros(shape).astype("float32"), ctx)
          ...
          beg = time.time()
          tg.run_task(sess, task_id, [inputs_bindings], profile_level=profile_level, save_to="")
          end = time.time()
          time_record.append((end - beg) * 1e3)

          outputs_data = tg.get_data_from_session(sess, output_keys)


          _, predict = torch.max(torch.from_numpy(outputs_data[0].asnumpy()), 1)
          num_correct = (predict == data[1]).sum()
          running_acc += num_correct.item()

          running_loss += outputs_data[1].asnumpy().item(0)
          if i % 100 == 0:
            print("Running loss:", running_loss / (batch * (i + 1)))
            print("Running accuracy:", running_acc / (batch * (i + 1)))

      print('Finish {} epoch, Acc: {:.6f}'.format(epoch + 1, running_acc / (len(train_dataset))))
      print("Time Record", time_record[:10])
  ...
  #   outputs_data = tg.get_data_from_session(sess, output_keys)
  #   print(outputs_data[0].asnumpy(), flush=True)
