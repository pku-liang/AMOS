...
def main_process(target, dev_id):
  ...
  # replay
  saved_file = "saved_schedules.txt"
  print("test schedules from", saved_file)
  tg.test_schedule_reference(sess, task_id, reference=saved_file)
...
@concurrent.process
def produce_process(target, dev_id):
  os.environ["TG_PRINT_LEVEL"] = "4"
  autoschedule_log_file="autoschedule_log.txt"
  autoschedule_profile_file="autoschedule_log_profile.txt"
  build_log_file="build_log.txt"
  ...
  # replay
  saved_file = "saved_schedules.txt"
  tg.begin_tuning(sess, task_id, 10)
  # execute graph by 'number' iterations
  number = 100
  repeats = 10
  for i in range(repeats):
    beg = time.time()
    tg.run_task(sess, task_id, [inputs_bindings] * number, save_to=saved_file)
    end = time.time()
    print("Average time cost for one iteration:", (end - beg) * 1e3 / number, "ms")
...
if __name__ == "__main__":
  ...
  producer_exit_code = produce_process(target, dev_id)
  try:
