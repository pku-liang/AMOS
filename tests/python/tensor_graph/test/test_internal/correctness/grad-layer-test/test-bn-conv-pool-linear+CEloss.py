...
def pytorch_result(img_np, label_np, params_np):
  ...
  weights_torch = []
  bn_torch = torch.nn.BatchNorm2d(channel, eps=1e-5, momentum=None, affine=True, track_running_stats=True)
  ...
  layers = [bn_torch, conv_torch, linear_torch]
  for i, l in enumerate(layers):
    if (i == 0):
      ...
      l.weight = torch.nn.Parameter(params_torch[i+1])
      l.zero_grad()
      weights_torch.append(l.weight)
    else:
      l.weight = torch.nn.Parameter(params_torch[i])
...
class MyModel(Layer):
  def __init__(self):
    ...
    self.bn_layer = BatchNorm2d(channel)
...
def test1():
  print("test 1 ##############################")
  model = MyModel()
  print("The parameters in bn-conv-pool-linear:")
