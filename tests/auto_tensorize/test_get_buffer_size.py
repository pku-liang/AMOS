...
from tvm.contrib import tar, ndk, nvcc
...
def test1():
    ...
    for name, func in ir_module.functions.items():
        buffer_size_map = at.get_buffer_size("shared", func.body)
        print(buffer_size_map)
        buffer_size_map = at.get_buffer_size("local", func.body)
        print(buffer_size_map)
