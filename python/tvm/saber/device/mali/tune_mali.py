...
class MaliProblemSizeParamGenerator(CDParamGenerator):
...
class MaliDeviceGeneralGenerator(SAEntryGenerator):
    def __init__(self, arch="g76", eps=0.9,
            log_file="mali_device_general_generator.log", steps=1):
        super(MaliDeviceGeneralGenerator, self).__init__(eps, MaliParams,
            steps=steps, log_file=log_file)
        self.init_param_generator()
        self.arch_info = at.target.Mali(arch=arch)
        self.init_score_table()

    def init_param_generator(self, *args):
        self.M = MaliProblemSizeParamGenerator(
                1, 256, 3
            )
        self.N = MaliProblemSizeParamGenerator(
                1, 256, 3
            )
        self.K = MaliProblemSizeParamGenerator(
                1, 256, 3
            )
        self.split_K = MaliProblemSizeParamGenerator(
                1, 1, 1
            )
        self.generator_lst = [
