    ...
    def __call__(self, input, kernel_size=None, stride=None):
        if self.layer_type == 'CONV':
            ...
            if not self.with_routing:
                ...
                for i in range(self.vec_len):
                    ...
                    with tf.compat.v1.variable_scope('ConvUnit_' + str(i)):
                        caps_i = tf.keras.layers.Conv2D(self.num_outputs, self.kernel_size, 
                                    strides=self.stride, activation=None)(input)
        ...
        if self.layer_type == 'FC':
            if self.with_routing:
                ...
                self.input = tf.reshape(input, shape=(batch_size, -1, 1, input.shape[-2], 1))

                with tf.compat.v1.variable_scope('routing'):
                    b_IJ = tf.constant(np.zeros([batch_size, input.shape[1], self.num_outputs, 1, 1], dtype=np.float32))
...
def routing(input, b_IJ):
    W_shape = (1, 1152, 10, 8, 16)
    W = tf.compat.v1.Variable(np.zeros(W_shape), shape=W_shape, dtype=tf.float32)
...
class CapsNet(object):
    def __init__(self, is_training=True):
        ...
        with self.graph.as_default():
            if is_training:
                ...
                self.X = tf.compat.v1.placeholder(tf.float32, shape=(batch_size, 28, 28, 1))
                self.labels = tf.compat.v1.placeholder(tf.int32, shape=(batch_size, ))
    ...
    def build_arch(self):
        with tf.compat.v1.variable_scope('Conv1_layer'):
            # Conv1, [batch_size, 20, 20, 256]
            conv1 = tf.keras.layers.Conv2D(256, kernel_size=9, activation='relu')(self.X)

            assert conv1.get_shape() == [batch_size, 20, 20, 256]

        # Primary Capsules layer, return [batch_size, 1152, 8, 1]
        with tf.compat.v1.variable_scope('PrimaryCaps_layer'):
...
if __name__ == "__main__":
    ...
    supervisor = tf.compat.v1.train.Supervisor(graph=model.graph)
