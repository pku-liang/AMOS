...
from jax.nn import sigmoid, elu, log_softmax
...
def LLTM(out_dim, num_classes, W_init=glorot_normal(), b_init=normal()):
    def init_fun(rng, input_shape):
        ...
        W, U, b = (
            W_init(k1, (input_shape[1], 3*out_dim)),
            W_init(k2, (out_dim, 3*out_dim)),
            b_init(k3, (3*out_dim,)),
        )
    ...
    def apply_fun(params, inp, **kwargs):
        ...
        interm = np.dot(inp, W) + np.dot(hidden, U) + b
        i, o, c = np.split(interm, 3, 1)
...
if __name__ == "__main__":
    ...
    init_fun, apply_LLTM = LLTM(num_hidden_units, num_classes)
    ...
    def update(i, opt_state, inputs, targets):
        ...
        return opt_update(i, grad(cross_entropy_loss)(params, inputs, targets, apply_LLTM), opt_state)
    ...
    logits = jit(apply_LLTM)(params, data)
