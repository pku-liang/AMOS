...
from tensor_graph.nn.functional import elementwise_add, mean
from tensor_graph.core import ForwardGraph, BackwardGraph, compute, \
                              GraphTensor, GraphOp, PyTIRGraph, make_fwd_graph, \
                              make_tir_graph
import math

def conv_bn(name, in_channel, out_channel, kernel_size, strides=1, padding=1, dilation=1, groups=1, use_bias=False):
    return Sequential(
    ...
    def __init__(self, name, in_channel, out_channel, strides, expand_ratio):
        ...
        if expand_ratio == 1:
            self.conv = Sequential(
                # depthwise
                Conv2d(hidden_dim, hidden_dim, 3, stride=strides, padding=1, groups=hidden_dim),
                BatchNorm2d(hidden_dim),
                ReLU(),
                # pointwise
                Conv2d(hidden_dim, out_channel, 1, stride=1, padding=0),
                BatchNorm2d(out_channel)
            )
        else:
            self.conv = Sequential(
                # pointwise
                Conv2d(in_channel, hidden_dim, 1, stride=1, padding=0),
                BatchNorm2d(hidden_dim),
                ReLU(),
                # depthwise
                Conv2d(hidden_dim, hidden_dim, 3, stride=strides, padding=1, groups=hidden_dim),
