...
python pytorch-train-languange-modeling.py \
    ...
    --wdrop 0 --dropoute 0 --dropouth 0 --dropouti0 --dropout 0 \
    --save PTBC.pt |& tee log
...
class RNNModel(nn.Module):
    ...
    def forward(self, input, hidden, return_h=False):
        ...
        emb = self.idrop(emb)
        print("in RNNModel forward----input:", input.size(), "emb:", emb.size())
        for i in range(len(hidden)):
            for j in range(len(hidden[i])):
                print("hidden[",i,"]", "[", j, "]", hidden[i][j].size())
        ...
        print("output", output.size(), "result", result.size())
        for i in range(len(raw_outputs)):
            print("raw_outputs[", i, "]", raw_outputs[i].size())
        for i in range(len(outputs)):
            print("outputs", "[", i, "]", outputs[i].size())
