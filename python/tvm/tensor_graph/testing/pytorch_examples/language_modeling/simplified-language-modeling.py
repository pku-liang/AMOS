...
CUDA_VISIBLE_DEVICES=6 python simplified-language-modeling.py \
    ...
    --save PTBC.pt
python simplified-language-modeling.py \
    --model GRU --nlayers 3 --emsize 200 --nhid 1000 --bptt 150 \
    --data data/pennchar --train_bs 128 \
    --optimizer adam --lr 2e-3 --epochs 20 --when 12 16 \
    --alpha 0 --beta 0 --wdecay 1.2e-6 --clip 0.25 \
    --wdrop 0.5 --dropoute 0 --dropouth 0.25 --dropouti 0.1 --dropout 0.1 \
    --save PTBC.pt
CUDA_VISIBLE_DEVICES=6 python simplified-language-modeling.py \
    --model MILSTM --nlayers 3 --emsize 200 --nhid 1000 --bptt 150 \
    --data data/pennchar --train_bs 128 \
    --optimizer adam --lr 2e-3 --epochs 20 --when 12 16 \
    --alpha 0 --beta 0 --wdecay 1.2e-6 --clip 0.25 \
    --wdrop 0.5 --dropoute 0 --dropouth 0.25 --dropouti 0.1 --dropout 0.1 \
    --save PTBC.pt
CUDA_VISIBLE_DEVICES=6 python simplified-language-modeling.py \
    --model SCRNN --nlayers 3 --emsize 200 --nhid 1000 --bptt 150 \
    --data data/pennchar --train_bs 128 \
    --optimizer adam --lr 2e-3 --epochs 20 --when 12 16 \
    --alpha 0 --beta 0 --wdecay 1.2e-6 --clip 0.25 \
    --wdrop 0.5 --dropoute 0 --dropouth 0.25 --dropouti 0.1 --dropout 0.1 \
...
class SplitCrossEntropyLoss(nn.Module):
    ...
    def __init__(self, hidden_size, splits, verbose=False):
        ...
        # hidden_size 200 splits [0, 100000000] self.nsplits 1 stats defaultdict(<class 'list'>, {})
        #print("hidden_size", hidden_size, "splits", self.splits, "self.nsplits", self.nsplits, "stats", self.stats)

    def split_on_targets(self, hiddens, targets):
        #split on targets hiddens torch.Size([19200, 200]) targets torch.Size([19200])
        #print("split on targets", "hiddens", hiddens.size(), "targets", targets.size())
    ...
    def forward(self, weight, bias, hiddens, targets, verbose=False):
        ...
        # weight torch.Size([50, 200])
        # targets torch.Size([19200])
        # hiddens.size()= torch.Size([19200, 200])
        ...
        #combo= torch.Size([19200, 200])
        # self.nsplits= 1 i= 0
        # split_hiddens[i]= torch.Size([19200, 200])
        combo = split_hiddens[0]
        #torch.cat([split_hiddens[i] for i in range(self.nsplits) if len(split_hiddens[i])])
        ###
        # for i in range(self.nsplits):
        #     print("self.nsplits=",self.nsplits, "i=", i)
        #     print("split_hiddens[i]=", split_hiddens[i].size())
        # print("combo=", combo.size())
        print("head_weight", head_weight.size())
        # combo= torch.Size([19200, 200])
        # head_weight torch.Size([50, 200]) ???Why [50, 1000]?
        # head_bias torch.Size([50])
        ...
        #softmaxed_all_head_res torch.Size([19200, 50])

        running_offset = 0
        softmaxed_head_res = softmaxed_all_head_res[running_offset:running_offset + len(split_hiddens[0])]
        entropy = -torch.gather(softmaxed_head_res, dim=1, index=split_targets[0].view(-1, 1))
        running_offset += len(split_hiddens[0])
...
class RNNFactory:
    ...
    def build(rnn_type, ntoken, ninp, nhid, nlayers,
        ...
        print("in build", "ntoken", ntoken, "ninp", ninp, "initrange", initrange, "nhid", nhid)
        print("wdrop", wdrop, "nlayers", nlayers)
        #in build ntoken 50 ninp 200 initrange 0.1 nhid 1000
        #wdrop 0.5 nlayers 3
        #ntoken 50 ninp 200 initrange 0.1
        #nhid 1000 ntoken 50 initrange 0.1

        encoder = RNNFactory._build_encoder(ntoken, ninp, initrange)
        decoder = RNNFactory._build_decoder(nhid, ntoken, initrange)
        print(decoder.weight.size(), "VS", encoder.weight.size())
        #torch.Size([50, 1000]) VS torch.Size([50, 200])
    ...
    def _build_encoder(ntoken, ninp, initrange):
        print("ntoken", ntoken, "ninp", ninp, "initrange", initrange)
        encoder = nn.Embedding(ntoken, ninp) #ntoken 50 ninp 200 initrange 0.1
        encoder.weight.data.uniform_(-initrange, initrange)
        return encoder

    @staticmethod
    def _build_decoder(nhid, ntoken, initrange):
        print("nhid", nhid, "ntoken", ntoken, "initrange", initrange) #nhid 1000 ntoken 50 initrange 0.1
    ...
    def __init__(self, ninp, nhid, nlayers, tie_weights):
        ...
        for l in range(nlayers):
            ...
            if l != nlayers - 1 or not tie_weights:
                ...
                hidden_size = ninp
            print("l=",l, "inpnut_size=", input_size, "hidden_size=", hidden_size)
...
class Dataset:
    def __init__(self, train, valid, test, train_bs, valid_bs, test_bs, ntokens):
        ...
        #train torch.Size([39199, 128]), test torch.Size([442424, 1]), train_bs 128, valid_bs 10, ntokens 50
        print("train", train.size(), "test", test.size(), "train_bs", train_bs, "valid_bs", valid_bs)
    ...
    def get_batch(source, i, seq_len):
        ...
        # data torch.Size([150, 128]) target torch.Size([19200])
        return data, target
...
class Learner:
    ...
    def train_one_epoch(self, epoch):
        ...
        while i < self.dataset.train.size(0) - 1 - 1:
            ...
            # train_one_epoch data.shape[0] 150 seq_len 150
            #print("train_one_epoch","data.shape[0]", data.shape[0], "seq_len", seq_len)
            if data.shape[0] < seq_len and self.config.drop_last:
                # print("data.shape[0] < seq_len and self.config.drop_last is true")
                # 48 < 150, drop_last is true
                break
