    ...
    if infer:
        def _inner_bn(N, C, H, W, inputs, alpha, beta, requires_grad=True):
            return compute([N, C, H, W],
                lambda n, c, i, j: inputs[n, c, i, j] * alpha[c] + beta[c],
                name=prefix + "_bn2d",
                # tag=tag_gen("bn2d"),
                requires_grad=requires_grad)
        ...
        def _inner_mean(C, N, H, W, inputs, requires_grad=False):
            ...
            return compute([C],
                ...
                requires_grad=requires_grad)
        mean = GraphOp([C], [N, H, W], [inputs], _inner_mean, name=prefix + "_mean", requires_grad=False)
        ...
        def _inner_square(C, N, H, W, inputs, requires_grad=False):
            ...
            return compute([N, C, H, W],
                lambda c: tvm.te.sum((inputs[rn2, c, rh2, rw2] * inputs[rn2, c, rh2, rw2]) / (N*H*W), axis=[rn2, rh2, rw2]),
                name=prefix + "_bn2d_square",
                # tag=tag_gen("bn2d_mean"),
                requires_grad=requires_grad)
        square = GraphOp([C], [N, H, W], [inputs], _inner_square, name=prefix + "_square", requires_grad=False)
        ...
        def _inner_var(C, square, mean, requires_grad=False):
            return compute([C],
                ...
                requires_grad=requires_grad)
        var = GraphOp([C], [], [square, mean], _inner_var, name=prefix + "_var", requires_grad=False)

        def _inner_bn(N, C, H, W, inputs, mean, var, alpha, beta, requires_grad=True):
            return compute([N, C, H, W],
                lambda n, c, i, j: (inputs[n, c, i, j] - mean[c]) / tvm.te.sqrt(var[c] + epsilon) * alpha[c] + beta[c],
                name=prefix + "_bn2d",
                # tag=tag_gen("bn2d"),
                requires_grad=requires_grad)
        #return (mean, var), (inputs_p, mean_p, var_p), bn
    ...
    if infer:
        def _inner_bn(N, C, D, H, W, inputs, alpha, beta, requires_grad=True):
            return compute([N, C, D, H, W],
                lambda n, c, d, i, j: inputs[n, c, d, i, j] * alpha[c] + beta[c],
                name=prefix + "_bn3d",
                # tag=tag_gen("bn2d"),
                requires_grad=requires_grad)
        #return (mean, var), (inputs_p, mean_p, var_p), bn
        ...
        def _inner_mean(C, N, D, H, W, inputs, requires_grad=False):
            ...
            rw1 = tvm.te.reduce_axis([0, W], name=prefix + "_rw1")
            return compute([C],
                lambda c: tvm.te.sum(inputs[rn1, c, rd1, rh1, rw1] / (N*D*H*W), axis=[rn1, rd1, rh1, rw1]),
                name=prefix + "_bn3d_mean",
                # tag=tag_gen("bn2d_mean"),
                requires_grad=requires_grad)
        mean = GraphOp([C], [N, D, H, W], [inputs], _inner_mean, name=prefix + "_mean", requires_grad=False)
        ...
        def _inner_square(C, N, D, H, W, inputs, requires_grad=False):
            ...
            return compute([N, C, D, H, W],
                ...
                # tag=tag_gen("bn2d_mean"),
                requires_grad=requires_grad)
        square = GraphOp([C], [N, D, H, W], [inputs], _inner_square, name=prefix + "_square", requires_grad=False)
        ...
        def _inner_bn(N, C, D, H, W, inputs, mean, var, alpha, beta, requires_grad=True):
            return compute([N, C, D, H, W],
                lambda n, c, d, i, j: (inputs[n, c, d, i, j] - mean[c]) / tvm.te.sqrt(var[c] + epsilon) * alpha[c] + beta[c],
                name=prefix + "_bn3d",
                # tag=tag_gen("bn2d"),
                requires_grad=requires_grad)
        #return (mean, var), (inputs_p, mean_p, var_p), bn
    ...
    if bias is not None:
        ...
        def _inner_bias_add(batch_size, out_channel, out_len, conved, bias, requires_grad=True):
            return compute([batch_size, out_channel, out_len],
                        ...
                        requires_grad=requires_grad)
        conved_bias = GraphOp([batch_size, out_channel, out_len], [], [conved, bias], _inner_bias_add, name="bias")
        return conved_bias
    return conved
...
def conv2d_capsule(inputs, weight, bias=None, stride=1, padding=0, dilation=1, groups=1, num_caps=8):
    """Convolution 2d NCHW layout
...
def conv2d_nchwc(inputs, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    conv_result = GraphOp([batch_size, out_channel_chunk, out_h, out_w, out_channel_block], 
            [channel_per_group_chunk, in_channel_block, k_w, k_h], [padded, weight],
...
def im2col_nchw_naive(inputs, kernel_size, stride=1, padding=0, dilation=1, groups=1):
    ...
    assert(all([isinstance(x, tvm.tir.IntImm) for x in kernel_size]))
    batch_size, channel, in_h, in_w = inputs.shape
    out_channel, channel_per_group, k_h, k_w = kernel_size
    out_channel_per_group = out_channel // groups
    assert(channel_per_group * groups == channel)
    assert(out_channel_per_group * groups == out_channel)
    ...
    padded = zero_pad2d(inputs,padding=padding)
    def _inner_im2col_nchw_naive(groups, out_height, output_width, padded, requires_grad=True):
        return compute([groups, out_height, output_width],
                ...
                name="im2col_nchw_naive",
                # tag=tag_gen("im2col_nchw_naive" + str(stride) + str(dilation) + str(groups)),
                requires_grad=requires_grad)
    return GraphOp([groups, out_height, output_width], [], [padded], _inner_im2col_nchw_naive, name="im2col_nchw_naive")
...
def col2img_nchw(inputs, P, Q):
    ...
    def _inner_col2img_nchw(batch_size, channel, P, Q, inputs, requires_grad=True):
        return compute([batch_size, channel, P, Q],
                lambda b, c, h, w: inputs[c // out_channel_per_group, c % out_channel_per_group, b * (P * Q) + h * Q + w],
                name="col2img_nchw",
                # tag=tag_gen("col2img_nchw"),
                requires_grad=requires_grad)
    return GraphOp([batch_size, out_channel_per_group * groups, P, Q], [], [inputs], _inner_col2img_nchw, name="col2img_nchw")
...
def gemm_conv2d_nchw(inputs, weight, bias=None, stride=1, padding=0, dilation=1, groups=1):
    ...
    def _inner_gemm_conv2d_nchw(groups, out_channel_per_group, col_shape2, col_shape1, 
            col, weight, requires_grad=True):
        k = tvm.te.reduce_axis((0, col_shape1))
        return compute([groups, out_channel_per_group, col_shape2],
                    lambda g, h, w: tvm.te.sum(col[g, k, w] * 
                                weight[g * out_channel_per_group + h, 
                                    ...
                                    k % k_w],
                                axis=k),
                    name="gemm_conv2d_nchw",
                    # tag=tag_gen("gemm_conv2d_nchw" + str(groups)),
                    requires_grad=requires_grad)
    gemm = GraphOp([groups, out_channel_per_group, col.shape[2]], [col.shape[1]], 
                [col, weight], _inner_gemm_conv2d_nchw, name="gemm_conv2d_nchw")
    if bias is not None:
        def _inner_bias_add(groups, out_channel_per_group, col_shape2, gemm, bias, requires_grad=True):
            return compute([groups, out_channel_per_group, col_shape2],
                    lambda g, h, w: gemm[g, h, w] + bias[g * out_channel_per_group + h],
                    name="gemm_conv2d_nchw_bias",
                    # tag=tag_gen("gemm_conv2d_nchw_bias"),
                    requires_grad=requires_grad)
        gemm_bias = GraphOp([groups, out_channel_per_group, col.shape[2]], [], [gemm, bias], 
                    _inner_bias_add, name="gemm_conv2d_nchw_bias")
        return col2img_nchw(gemm_bias, P, Q)
    ...
    if bias is not None:
        ...
        def _inner_bias_add(*args, requires_grad=True):
            return compute(args[-2].shape, 
                        lambda *indice: args[-2][indice] + args[-1](indice[-1]),
                        name="linear_bias",
...
def bilinear(inputs1, inputs2, weight, bias=None):
    ...
    def _inner_bilinear(*args, requires_grad=True):
        def _inner(*indices):
            return tvm.te.sum(
                inputs1[(*indices[:-1], k1)] * weight[indices[-1], k1, k2] * inputs2[(*indices[:-1], k2)], 
                axis=[k1, k2]
                )
...
def MTTKRP3d(A, B, C):
    ...
    def _inner_MTTKRP3d(A_shape0, B_shape1, B_shape0, C_shape0, A, B, C, requires_grad=True):
        return compute([A_shape0, B_shape1],
                lambda i, j: tvm.te.sum(A[i, k, l] * B[k, j] * C[l, j], axis=[k, l]),
                name="MTTKRP3d",
...
def LSTMCell(inputs, hs, cs, weights, bias=None):
    ...
    def _inner_next_cs(cs_0, weight_1, C, cs, requires_grad=True):
        return compute([cs_0, weight_1],
                ...
                # tag=tag_gen("next_cs"),
                requires_grad=requires_grad)
    next_cs = GraphOp([cs.shape[0], weights.shape[1]], [], [C, cs], _inner_next_cs, name="next_cs")
    ...
    def _inner_next_hs(hs_0, weight_1, C, next_cs, requires_grad=True):
        return compute([hs_0, weight_1],
                ...
                # tag=tag_gen("next_hs"),
                requires_grad=requires_grad)
    next_hs = GraphOp([hs.shape[0], weights.shape[1]], [], [C, next_cs], _inner_next_hs, name="next_hs")
...
def block_circulant_matrix(Input, factor):
    ...
    def _inner_compress(shape0, shape1, FFT, Input, requires_grad=True):
        return compute([shape0, shape1],
                ...
                requires_grad=requires_grad)
    Compress = GraphOp([ROW // FFT, (COL // FFT) * FFT], [FFT], [Input], _inner_compress, name="block_circulant_matrix_compress")
    ...
    Output = GraphOp([ROW, COL], [], [Compress], _inner_output, name="block_circulant_matrix_output")
    ...
    if mask_type == 'A':
        def _inner_PixelCNN_A(batch, out_c, kernel_h, kernel_w, Kernel, requires_grad=True):
            return compute([batch, out_c, kernel_h, kernel_w],
                    ...
                    requires_grad=requires_grad)
        Mask = GraphOp(Kernel.shape, [], [Kernel], _inner_PixelCNN_A, name="PixelCNN_A")
    else:
        def _inner_PixelCNN_B(batch, out_c, kernel_h, kernel_w, Kernel, requires_grad=True):
            return compute([batch, out_c, kernel_h, kernel_w],
                    ...
                    requires_grad=requires_grad)
        Mask = GraphOp(Kernel.shape, [], [Kernel], _inner_PixelCNN_B, name="PixelCNN_B")
...
def GatedPixelCNN(Input, KernelV, KernelV2H, KernelH, KernelHOut, ClassVector=None, bias=None, dilation=1, stride=1, padding=0):
    ...
    def _inner_CombineFeature(ConvH_0, ConvH_1, ConvH_2, ConvH_3, ConvH, Vertical2HorizonTal, requires_grad=True):
        return compute([ConvH_0, ConvH_1, ConvH_2, ConvH_3],
                ...
                requires_grad=requires_grad)
    CombineFeature = GraphOp(ConvH.shape, [], [ConvH, Vertical2HorizonTal], _inner_CombineFeature, name="GatedPixelCNN_CombineFeature")
    ...
    if ClassVector == None:
        def _inner_ActivationV_1(ConvV_0, ConvV_1, ConvV_2, ConvV_3, ConvV, requires_grad=True):
            return compute([ConvV_0, ConvV_1, ConvV_2, ConvV_3],
                    ...
                    requires_grad=requires_grad)
        ActivationV = GraphOp(ConvV.shape, [], [ConvV], _inner_ActivationV_1, name="GatedPixelCNN_ActivationV_1")
    else:
        def _inner_ActivationV_2(ConvV_0, ConvV_1, ConvV_2, ConvV_3, ConvV, ClassVector, requires_grad=True):
            return compute([ConvV_0, ConvV_1, ConvV_2, ConvV_3],
                    ...
                    requires_grad=requires_grad)
        ActivationV = GraphOp(ConvV.shape, [], [ConvV, ClassVector], _inner_ActivationV_2, name="GatedPixelCNN_ActivationV_2")
    ...
    def _inner_ActivationH(combine_0, combine_1, combine_2, combine_3, CombineFeature, requires_grad=True):
        return compute([combine_0, combine_1, combine_2, combine_3],
                ...
                requires_grad=requires_grad)
    ActivationH = GraphOp(CombineFeature.shape, [], [CombineFeature], _inner_ActivationH, name="ActivationH")
    ...
    def _inner_Output(shape0, shape1, shape2, shape3, ConvGateH, Input, requires_grad=True):
        return compute([shape0, shape1, shape2, shape3],
                ...
                requires_grad=requires_grad)
    Output = GraphOp(ConvGateH.shape, [], [ConvGateH, Input], _inner_Output, name="GatedPixelCNN_output")
...
def ReLU(x):
    ...
    def _inner_ReLU(*args, requires_grad=True):
        ...
        return compute(
            args[:-1], lambda *i: tvm.te.max(args[-1](*i), tvm.tir.const(0, args[-1].tvm_tensor.dtype)),
            name="relu",
...
def dense(inputs, weight, bias=None):
    ...
    def _inner_dense(batch, out_feature, in_feature, inputs, weight, requires_grad=True):
        ...
        return compute(
            [batch, out_feature], 
            lambda i, j: tvm.te.sum(inputs[i, k] * weight[j, k], axis=[k]),
            name="dense",
            # tag=tag_gen("dense"),
            requires_grad=requires_grad)
    
    withoutBias = GraphOp([batch, out_feature], [in_feature], [inputs, weight], _inner_dense, name="linear")
...
def avgpool2d(inputs, kernel_h=2, kernel_w=2, stride_h=2, stride_w=2, padding=0):
    ...
    def _inner_avgpool2d(batch, channel, h_out, w_out, kernel_h, kernel_w, padded_inputs, requires_grad=True):
        ...
        return compute(
            ...
            lambda n, c, i, j: 
                tvm.te.sum(padded_inputs[n, c, i * stride_h + r, j * stride_w + s]/(kernel_h*kernel_w), axis=[r, s]), 
            name="avgpool2d",
...
def global_avg_pool2d(inputs, keep_dim=True):
    ...
    def _innner_global_avg_pool2d_keep(N, C, one1, one2, H, W, inputs, requires_grad=True):
        ...
        return compute([N, C, one1, one1],
            lambda n, c, i, j:
                tvm.te.sum(inputs[n, c, i + h, j + w]/(H*W), axis=[h, w]),
...
def global_avg_pool3d(inputs, keep_dim=True):
    ...
    def _innner_global_avg_pool3d_keep(N, C, one1, one2, one3, D, H, W, inputs, requires_grad=True):
        ...
        return compute([N, C, one1, one2, one3],
            lambda n, c, l, i, j:
                tvm.te.sum(inputs[n, c, l + d, i + h, j + w]/(H*D*W), axis=[d, h, w]),
    ...
    def _innner_global_avg_pool3d_nokeep(N, C, D, H, W, inputs, requires_grad=True):
        ...
        return compute([N, C],
            lambda n, c:
                tvm.te.sum(inputs[n, c, d, h, w]/(H*D*W), axis=[d, h, w]),
