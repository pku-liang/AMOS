...
def empty_tenet_state():
    return TenetState(set(), [], [], [], {})
...
#####################################################
class TenetKernelParamGenerator(CDParamGenerator):
    pass
...
class TenetScheduleGenerator(AcceleratorScheduleGenerator):
    def __init__(self, intrin_match_result, transform_state, eps=0.7,
        ...
        self.arch_info = TENET(arch=arch)
...
class TenetScheduleApplier(object):
    def __init__(self, intrin_match_result, schedule_compute_info, arch=""):
        ...
        self.params = empty_tenet_params()
        # some constants
    ...
    def get_main_op_thread_numbers(self):
    ...
    def cache_read(self, op_id, op, sch, X):
        ...
        if do_cache_read_for_load:
            ...
            thread_num = self.get_main_op_thread_numbers()
            ...
            fused, thread_level = sch[S].split(fused, factor=thread_num)
    ...
    def tiling(self, op_id, op, sch, X):
        ...
        if op == self.main_op:
            ...
            sch[X(op)].bind(fused_axis[-1], self.tx)
            ...
            if op != self.output_op:
                ...
                split_factors = self.get_last_op_axis_factors()
                split_parts = []
                ...
                sch[X(op)].bind(split_parts[-1], self.otx)
                # sch[X(op)].bind(thread_level, self.otx)
                self.state.last_op_axis = [[x] for x in split_parts]
        else:
            ...
            if do_tiling:
                ...
                sch[X(op)].bind(split_parts[-1], tx)
