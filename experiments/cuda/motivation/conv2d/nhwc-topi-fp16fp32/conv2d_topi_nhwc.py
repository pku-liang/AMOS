...
from tvm.topi.util import get_const_tuple, traverse_inline, simplify
...
tag_name = "motivation_conv2d_nhwc"
...
def autotvm_conv2d_tensorcore_nhwc(
    N, C, H, W, K, R, S, stride=1, padding=0, dilation=1,
        dtype="float16", out_dtype="float32"):
    Input = tvm.te.placeholder([N, H, W, C], dtype=dtype, name="Input")
    Filter = tvm.te.placeholder([R, S, C, K], dtype=dtype, name="Filter")
    cfg = autotvm.get_config()
    Conv = nhwc_tensorcore_cuda(cfg, Input, Filter, stride, padding, dilation, out_dtype)
    sch = tvm.te.create_schedule(Conv.op)
    schedule_nhwc_tensorcore_cuda(cfg, sch, Conv)
    return sch, [Input, Filter, Conv]


schedule_func = autotvm_conv2d_tensorcore_nhwc
