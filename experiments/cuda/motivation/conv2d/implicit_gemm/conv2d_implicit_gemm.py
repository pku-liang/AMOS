...
def intrin_wmma_load_matrix_a(dtype):
...
def intrin_wmma_load_matrix_b(dtype):
...
def intrin_wmma_gemm(out_strides, dtype, out_dtype):
    n = 16
...
def intrin_wmma_store_matrix(in_strides, out_strides, out_dtype):
    n = 16
...
def schedule_conv2d_implicit_gemm(
    ...
    sch[Conv].tensorize(
        nhwi, intrin_wmma_store_matrix([warp_gemm[1], 1], [K, 1], out_dtype))
    ...
    OL_crso = crso
    sch[OL].tensorize(nhwi, intrin_wmma_gemm([warp_gemm[1], 1], dtype, out_dtype))
    ...
    sch[AL].tensorize(
        nhwi, intrin_wmma_load_matrix_a(dtype))
    ...
    sch[BL].tensorize(
        ki, intrin_wmma_load_matrix_b(dtype))
...
def run(N, C, H, W, K, R, S, stride=1, padding=0, dtype="float16", out_dtype="float32", cfg=None):
...
def brute_force_run(N, C, H, W, K, R, S, stride=1, padding=0, dtype="float16", out_dtype="float32"):
    config_lst = []
    m = 4
    extent = 10
    ...
    with open("brute_force_tune_implicit_gemm.log", "w") as fout:
        for cfg in config_lst:
            try:
                cost = run(N, C, H, W, K, R, S, stride=1, padding=0, dtype="float16", out_dtype="float32", cfg=cfg)
                if cost < min_cost:
                    min_cost = cost
                    print(
...
def autotvm_tune(N, C, H, W, K, R, S, stride=1, padding=0, dtype="float16", out_dtype="float32"):
    ...
    task_conv = autotvm.task.create(
        "tutorial/conv2d_implicit_gemm_tensorcore", args=(
            N, C, H, W, K,
            R, S, stride, padding, dtype, out_dtype),
            target="cuda"
    )
