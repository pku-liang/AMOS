...
def conv2d(N, C, H, W, K, R, S, stride, padding, dilation):
    ...
    Conv = tvm.te.compute(
        [N, K, P, Q],
        lambda n, k, p, q:
            tvm.te.sum((Pad[n, rc, p*stride+rr, q*stride+rs].astype("int32") * B[k, rc, rr, rs].astype("int32")
                        ), axis=[rc, rr, rs]),
        name="Conv"
    )
...
def tensorize_avx512_u8s8s32(
    ...
    log_file = "conv2d-u8s8s32-layer-%d-batch-%d.log" % (layer, N)
    ...
    result = at.auto_tensorize_v3(
        ...
        build_parallel=32, run_parallel=4)
    # result = at.auto_tensorize(
    #     target_dag, target, log_file, measure_opt,
    #     trials=trials, search_batch=16,
    #     verbose=False, transform_dump=False)
...
def run(N, C, H, W, K, R, S, stride,
        padding, dilation, layer):
    return tensorize_avx512_u8s8s32(
        N, C, H, W, K, R, S, stride,
        padding, dilation, layer)
...
res18_shapes_b1 = [
    # resnet-18
    # (1, 128, 14, 14, 64, 128, 3, 3, 1, 1, 1, 1, 1),
