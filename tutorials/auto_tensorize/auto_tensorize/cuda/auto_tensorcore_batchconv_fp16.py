...
def batch_conv2d(N, C, H, W, K, R, S, stride, padding, dilation):
    pH = H + 2 * padding
    pW = W + 2 * padding
    A = tvm.te.placeholder([N, C, H, W], dtype="float16", name="A")
    B = tvm.te.placeholder([N, K, C, R, S], dtype="float16", name="B")
    ...
    Conv = tvm.te.compute(
        [N, K, P, Q],
        lambda n, k, p, q:
            tvm.te.sum((Pad[n, rc, p*stride+rr, q*stride+rs] * B[n, k, rc, rr, rs]
...
def tensorize_tensorcore_fp16fp16(
    ...
    A, B, Conv = batch_conv2d(N, C, H, W, K, R, S, stride, padding, dilation)
    target_dag = at.compute_dag_from_tensors([Conv])
    target = "cuda"

    log_file = "batch-conv2d-fp16-layer-%d-batch-%d.log" % (layer, N)
