...
def conv2d_nchw(N, C, H, W, K, R, S, CI, KI, stride, padding, dilation):
    ...
    Output = tvm.te.compute(
        ...
        lambda n, ko, p, q, ki:
            tvm.te.sum(
                padded[n, rco, p * stride + rr * dilation, q * stride + rs * dilation, rcie * ELE + rele].astype("int32")
...
def run(N, C, H, W, K, R, S, CI, KI, stride, padding, dilation):
    (A, B), (Output,) = conv2d_nchw(N, C, H, W, K, R, S, CI, KI, stride, padding, dilation)
...
def verify(N, C, H, W, K, R, S, CI, KI, stride, padding, dilation):
    ...
    A_np = np.random.uniform(-10, 10, [int(x) for x in A.shape]).astype("uint8")
    B_np = np.random.uniform(-10, 10, [int(x) for x in B.shape]).astype("int8")
    C_np1 = np.random.uniform(-10, 10, [int(x) for x in Output.shape]).astype("int32")
    C_np2 = np.random.uniform(-10, 10, [int(x) for x in Output.shape]).astype("int32")
