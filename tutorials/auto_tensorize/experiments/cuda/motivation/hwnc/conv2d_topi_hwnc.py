...
from tvm.topi.util import get_const_tuple, traverse_inline, simplify, tag
from tvm.topi.nn.pad import pad
from tvm.topi.nn.util import get_pad_tuple
from tvm.topi.cuda.tensor_intrin import intrin_wmma_load_matrix_A
from tvm.topi.cuda.tensor_intrin import intrin_wmma_load_matrix_W
from tvm.topi.cuda.tensor_intrin import intrin_wmma_store_matrix
from tvm.topi.cuda.tensor_intrin import intrin_wmma_gemm


tag_name = "motivation_conv2d_hwnc"
...
def conv2d_hwnc_tensorcore(cfg, data, kernel, strides, padding, dilation, in_dtype, out_dtype="int32"):
    ...
    assert data.dtype in ("int4", "uint4", "int8", "uint8"), data.dtype
    assert kernel.dtype in ("int4", "uint4", "int8", "uint8")
    packed_out = hwnc_tensorcore_cuda(cfg, data, kernel, strides, padding, dilation, out_dtype)
...
def hwnc_tensorcore_cuda(cfg, Input, Filter, stride, padding, dilation, out_dtype="int32"):
    ...
    assert isinstance(dilation, int) or len(dilation) == 2, dilation
...
@autotvm.template(tag_name)
def autotvm_conv2d_tensorcore_hwnc(
    N, C, H, W, K, R, S, stride=1, padding=0, dilation=1,
        dtype="float16", out_dtype="float32"):
    Input = tvm.te.placeholder([H, W, N, C], dtype=dtype, name="Input")
    Filter = tvm.te.placeholder([R, S, K, C], dtype=dtype, name="Filter")
    cfg = autotvm.get_config()
    Conv = hwnc_tensorcore_cuda(cfg, Input, Filter, stride, padding, dilation, out_dtype)
    sch = tvm.te.create_schedule(Conv.op)
    schedule_hwnc_tensorcore_cuda(cfg, sch, Conv)
    return sch, [Input, Filter, Conv]


schedule_func = autotvm_conv2d_tensorcore_hwnc


def autotvm_run(
    N, C, H, W, K, R, S, stride=1, padding=0, dilation=1,
        dtype="float16", out_dtype="float32", log_name=tag_name+".log"):
    logging.getLogger("autotvm").setLevel(logging.DEBUG)
    logging.getLogger("autotvm").addHandler(logging.StreamHandler(sys.stdout))

    task = autotvm.task.create(
        tag_name, args=(N, C, H, W, K, R, S, stride, padding, dilation, dtype, out_dtype), target="cuda"
    )
    ...
    with autotvm.apply_history_best(log_name):
        with tvm.target.Target("cuda"):
            s, arg_bufs = schedule_func(
                N, C, H, W, K, R, S, stride, padding, dilation, dtype, out_dtype)
            func = tvm.build(s, arg_bufs)

    # check correctness
    a_np = np.random.uniform(size=[int(x) for x in arg_bufs[0].shape]).astype(dtype)
    w_np = np.random.uniform(size=[int(x) for x in arg_bufs[1].shape]).astype(dtype)
    c_np = np.random.uniform(size=[int(x) for x in arg_bufs[2].shape]).astype(dtype)
    ...
    print("Time cost of this operator: %f ms" % cost)
    return cost
