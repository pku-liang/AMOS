...
# https://github.com/apache/incubator-tvm/pull/5601/files
...
def np_float2tvm_bf16(arr, ctx):
    """Convert a numpy array of float to a TVM array
    of bf16"""
    nparr = np_float2np_bf16(arr)
    return tvm.nd.empty(nparr.shape, "uint16", ctx=ctx).copyfrom(nparr)
...
if nvcc.have_tensorcore(ctx.compute_version):
    ...
    w_np = np.random.uniform(size=kernel_shape).astype("float32")

    a = np_float2tvm_bf16(a_np, ctx)
    w = np_float2tvm_bf16(w_np, ctx)
