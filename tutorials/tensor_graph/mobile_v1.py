...
def test1():
    ...
    out_dtype = "float16"
    target = "cuda"
    model = tensor_graph.testing.models.MobileNetv1(dtype=dtype, out_dtype=out_dtype)
    model.eval()
    ...
    tid = dispatch.add_graph_task(
        "mobile_v1", multi_graph, measure_opt, scheduler_option="auto_tensorize", trials=200)
    for i in range(10):
