...
@autotvm.template("conv2d_hwnc_tensorcore_test")
def conv2d_hwnc_tensorcore_test(N, H, W, CO, CI, KH, KW, stride, padding):
    data = tvm.te.placeholder((H, W, N, CI), name='data', dtype="int8")
    kernel = tvm.te.placeholder((KH, KW, CO, CI), name='kernel', dtype="int8")
    cfg = autotvm.get_config()

    conv = topi.cuda.hwnc_tensorcore_cuda(
        cfg, data, kernel, stride, padding, 1, 'int32')
    s = tvm.te.create_schedule([conv.op])
    topi.cuda.schedule_hwnc_tensorcore_cuda(cfg, s, conv)
...
def run(name, N, H, W, CO, CI, KH, KW, stride, pad):
    ...
    task = autotvm.task.create("conv2d_hwnc_tensorcore_test",
    ...
    with autotvm.apply_history_best(logfile):
        with tvm.target.create("cuda"):
            s, arg_bufs = conv2d_hwnc_tensorcore_test(N, H, W, CO, CI, KH, KW, strides, padding)
    ...
    ctx = tvm.gpu()
    a_tvm = tvm.nd.empty([H, W, N, CI], dtype="int8", ctx=ctx)
    w_tvm = tvm.nd.empty([KH, KW, CO, CI], dtype="int8", ctx=ctx)
    c_tvm = tvm.nd.empty([int(x) for x in arg_bufs[-1].shape], dtype="int32", ctx=ctx)
