...
std::future<tvm::runtime::Module> FunctionBuilder::build_for(
  tvm::te::Schedule sch,
  const tvm::Array<tvm::te::Tensor>& args,
  const tvm::Target& target,
  const tvm::Target& target_host,
  ...
  if (priority == 0) {
    return ThreadPool::Global().push_back(build_func, sch, args, target, target_host, name, binds, config);
  } else if (priority == 1) {
    // high priority
    return ThreadPool::Global().push_front(build_func, sch, args, target, target_host, name, binds, config);
  } else {
